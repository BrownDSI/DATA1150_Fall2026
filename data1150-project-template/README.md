...
> **Read this first:** Never commit raw, sensitive, or identifiable data (student records, course evaluations, chat transcripts, credentials, API keys). See [Data and privacy rules](#data-and-privacy-rules).
...
1. Click **Fork** at the top right of this repo on GitHub. Set the fork to **private** if your organization allows it.
2. Clone your fork to your computer:
   ```bash
   git clone https://github.com/<your-username>/<repo-name>.git
   cd <repo-name>
   ```
3. (Optional) Add the course repo as `upstream` so you can pull in fixes from the TA:
   ```bash
...
- If you accidentally commit sensitive data, **tell the TA right away.** Deleting it in a later commit does not remove it from Git history.
